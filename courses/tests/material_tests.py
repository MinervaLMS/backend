...
class UpdateMaterialTestCase(TestCase):
    ...
    def test_update_material_order(self):
        response = self.client.patch(
            f'/material/update/{self.material.id}/', self.material_update_order, format='json')
    ...
    def test_update_material_order(self):
        response = self.client.patch(
            f'/material/update/{self.material.id}/', self.material_invalid_fields, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(loads(response.content),  {
                         "message": f"last_name attribute does not exist in material"})

    def test_update_material_correct(self):
        response = self.client.patch(
            f'/material/update/{self.material.id}/', self.material_update_data, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class DeleteMaterialTestCase(TestCase):
    ...
    def test_material_delete_not_exist(self):
        response = self.client.delete(
            f'/material/delete/{self.material.id + 1}/')
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
