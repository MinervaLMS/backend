...
from ..schemas import register_schemas as schemas
from ..serializers.user_serializer import UserSerializer
from ..helpers.token_generator import confirmation_token_generator
from ..helpers.emails import send_confirmation_email
...
def user_register(request) -> JsonResponse:
    ...
    try:
        ...
        if not settings.DEBUG:
            return JsonResponse(
                {"uidb64": uidb64, "token": token}, status=status.HTTP_201_CREATED
            )
...
def confirm_email(request, uidb64: str, token: str) -> JsonResponse:
    ...
    return JsonResponse(
        {"message": "Email was confirmed successfully"}, status=status.HTTP_200_OK
    )
...
def resend_confirmation_email(request, uidb64: str) -> JsonResponse:
    ...
    try:
        ...
        return JsonResponse(
            {"message": "Email was sent"}, status=status.HTTP_201_CREATED
        )
    except Exception:
        return JsonResponse(
            {"message": "Email was not sent"},
