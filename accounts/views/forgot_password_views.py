...
from ..schemas import forgot_password_schemas as schemas
...
def forgot_my_password(request) -> JsonResponse:
    ...
    if not user_using:
        return JsonResponse(
            {"message": "No user with this email."}, status=status.HTTP_404_NOT_FOUND
        )
    ...
    try:
        emails.send_forgot_email(email, token, uidb64)
    except Exception:
        return JsonResponse(
...
def modify_forgotten_password(request, uidb64: str, token: str) -> JsonResponse:
    ...
    if new_password == "":
        return JsonResponse(
            {"message": "Password is not valid"}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    return JsonResponse(
        {"message": "Password was changed successfully"}, status=status.HTTP_200_OK
    )
